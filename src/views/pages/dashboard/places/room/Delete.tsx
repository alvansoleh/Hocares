...
const VPageDashboardPlaceRoomDelete: React.FC<MComponentUsecasePlaceRoom.Delete> = ({
    isOpen,
    loading,
    value,
    onClose,
    ...
    return (<VComponentModal
        isOpen={isOpen}
        size="sm"
        title={CUtilityString.title.dashboard.places.room.delete}
        content={(<Box width={"full"}>
            {value !== undefined && (<Text fontSize={14} textColor={CUtilityColor.gray} dangerouslySetInnerHTML={{ 
                __html: CUtilityString.messages.dashboard.places.room.delete(value.nama_gedung)
            }}  />)}
        </Box>)}
...
export default VPageDashboardPlaceRoomDelete