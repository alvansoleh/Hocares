...
const VPageDashboardPlaceBuildingDelete: React.FC<MComponentUsecasePlaceBuilding.Delete> = ({
    isOpen,
    loading,
    value,
    onClose,
    ...
    return (<VComponentModal
        isOpen={isOpen}
        size="sm"
        title={CUtilityString.title.dashboard.places.building.delete}
        content={(<Box width={"full"}>
            {value !== undefined && (<Text fontSize={14} textColor={CUtilityColor.gray} dangerouslySetInnerHTML={{ 
                __html: CUtilityString.messages.dashboard.places.building.delete(value.nama_gedung)
            }}  />)}
        </Box>)}
...
export default VPageDashboardPlaceBuildingDelete