...
const VPageDashboardAssetDelete: React.FC<MComponentUsecaseAsset.Delete> = ({
    isOpen,
    loading,
    value,
    onClose,
    ...
    return (<VComponentModal
        isOpen={isOpen}
        size="sm"
        title={CUtilityString.title.dashboard.asset.delete}
        content={(<Box width={"full"}>
            {value !== undefined && (<Text fontSize={14} textColor={CUtilityColor.gray} dangerouslySetInnerHTML={{ 
                __html: CUtilityString.messages.dashboard.asset.delete(value.nama_aset)
            }}  />)}
        </Box>)}
...
export default VPageDashboardAssetDelete