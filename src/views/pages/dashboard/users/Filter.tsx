...
import { CUsecaseUser, CUtilityString } from "controllers";
import { useEffect, useState } from "react";
import { VComponentFormFilterBy, VComponentFormSearch, VComponentFormSortBy, VComponentModal } from "views/components";

const VPageDashboardUserFilter: React.FC<MComponentGlobalSearch.ContainerQuery> = ({
    isOpen,
    loading,
    value,
    onClose,
    ...
    useEffect(() => {
        if (value !== undefined && isOpen) {
            ...
            CUtilityString.forms.filter_by.drop_down.user.forEach((it, i) => {
    ...
    return (<VComponentModal
        isOpen={isOpen}
        size="sm"
        title={CUtilityString.title.dashboard.user.filter}
        content={(<VStack spacing={2} width={"full"}>
            <VComponentFormFilterBy
                active_index={filterByActiveIndex}
                options={CUtilityString.forms.filter_by.drop_down.user}
                onSelect={(i, option) => {
...
export default VPageDashboardUserFilter