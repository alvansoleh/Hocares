...
const VPageDashboardUserAccessDelete: React.FC<MComponentUsecaseUserAccess.Delete> = ({
    isOpen,
    loading,
    value,
    onClose,
    ...
    return (<VComponentModal
        isOpen={isOpen}
        size="sm"
        title={CUtilityString.title.dashboard.user_access.delete}
        content={(<Box width={"full"}>
            {value !== undefined && (<Text fontSize={14} textColor={CUtilityColor.gray} dangerouslySetInnerHTML={{ 
                __html: CUtilityString.messages.dashboard.user_access.delete(value.id_menu)
            }}  />)}
        </Box>)}
...
export default VPageDashboardUserAccessDelete